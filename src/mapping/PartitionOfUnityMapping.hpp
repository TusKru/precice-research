...
void PartitionOfUnityMapping<RADIAL_BASIS_FUNCTION_T>::computeMapping()
{
  ...
  auto [anisotropicClusters, clusterRange] = std::move(createAnisotropicClustering<RADIAL_BASIS_FUNCTION_T>(inMesh, outMesh, _basisFunction, _polynomial, _verticesPerCluster));
  eClusters.stop();

  _anisotropicClusters = std::move(anisotropicClusters);
  _clusterRange = clusterRange;
...
void PartitionOfUnityMapping<RADIAL_BASIS_FUNCTION_T>::tagMeshFirstRound()
{
  ...
  if (_clusterRange == 0)
    _clusterRange = impl::estimateClusterRadius(_verticesPerCluster, filterMesh, localBB);

  PRECICE_DEBUG("Cluster radius estimate: {}", _clusterRange);
  PRECICE_ASSERT(_clusterRange > 0);

  // Now we extend the bounding box by the radius
  localBB.expandBy(2 * _clusterRange);
...
void PartitionOfUnityMapping<RADIAL_BASIS_FUNCTION_T>::exportClusterCentersAsVTU(mesh::Mesh &centerMesh)
{
  ...
  dataRadius->values().fill(_clusterRange);
  for (unsigned int i = 0; i < _anisotropicClusters.size(); ++i) {
